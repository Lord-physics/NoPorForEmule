...
Cierra NoPorForEmule y ejecuta **Actualizar.bat** en la carpeta donde lo instalaste. Descarga una revisión concreta de la rama `main` de GitHub y reemplaza los archivos del programa después de verificar el ZIP. Si falla la sustitución, restaura los archivos anteriores. Conserva `.venv`, los archivos ajenos al programa y la configuración en `%LOCALAPPDATA%`. Si cambian las dependencias, las instala en el entorno virtual antes de sustituir el código. Necesita conexión y que el repositorio sea público. Si lo descargaste como ZIP, vuelve a extraer el ZIP del repositorio una vez para disponer de `Actualizar.bat`.
